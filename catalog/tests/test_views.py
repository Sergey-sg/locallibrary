...
class AuthorCreateViewTest(TestCase):
    ...
    def test_redirect_if_added_successfully(self):
        login = self.client.login(username='testuser2', password='12345')
        # resp = self.client.get(reverse('author-create'))
        # self.assertEqual(resp.status_code, 200)
        form_data = {'first_name': 'test', 'last_name': 'test', 'date_of_birth': '1988-03-03'}
        resp = self.client.post(reverse('author-create'), form_data)
        self.assertTrue(resp.url.startswith('/catalog/author/1'))
